...
/*   Updated: 2022/12/13 18:53:30 by jocardos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	config_params(t_const_philo *var, char **argv)
{
	...
	if (var->num_philo <= 0 || var->time_die <= 0 || var->time_eat <= 0
		|| var->time_sleep <= 0)
		return (1);
	if (argv[5])
	{
...
void	assign_params(t_philo *philo, t_const_philo *var, pthread_mutex_t *m,
		pthread_mutex_t *pr)
{
	int	i;
